...
export class WardBillItemRequestComponent {
  ...
  ngOnInit() {
    ...
    if (this.patientId && this.visitId) {
      this.billingBLService.GetDataOfInPatient(this.patientId, this.visitId)
        .subscribe((res: DanpheHTTPResponse) => {
          if (res.Status == "OK") {
            this.currPatVisitContext = res.Results;
          } else {
            this.msgBoxServ.showMessage("failed", ["Problem! Cannot get the Current Visit Context ! "], res.ErrorMessage)
          }
        });
  ...
  ResetDoctorListOnItemChange(item, index) {
    ...
    if (item) {
      let docArray = null;
      let currItemPriceCFG = this.billItems.find(a => a.ItemId == item.ItemId && a.ServiceDepartmentId == item.ServiceDepartmentId);
      if (currItemPriceCFG) {
