...
@Component({
  templateUrl: "../../view/pharmacy-view/Sale/PHRMSale.html", //  "/PharmacyView/PHRMSale",
...
export class PHRMSaleComponent {
  ...
  getPharmaPatientList() {
    this.pharmacyBLService.GetPatientsListForSaleItems()
      .subscribe(res => {
        if (res.Status == 'OK') {
          this.patientListAutoComplete = res.Results;
        }
      });
  }
  ...
  CallBackSaveSale(res) {
    try {
      if (res.Status == "OK") {
        ...
        txnReceipt.Patient.CountrySubDivisionName = this.currSale.selectedPatient.CountrySubDivisionName;
        txnReceipt.Patient.DateOfBirth = this.currSale.selectedPatient.DateOfBirth;
  ...
  public SetAnonymous() {
    try {
      ...
      this.currSale.selectedPatient.Gender = 'Others';
      ...
      this.currSale.selectedPatient.Age = '26Y';
    ...
    if($event.PatientId>0  || $event.PatientId == -1){
      ...
      .subscribe(res => {
        if (res.Status == "OK") {
          ...
          patient.ShortName = res.Results.FirstName + ((res.Results.MiddleName !=null)?(' ' + res.Results.MiddleName + ' ') : (' ')) + res.Results.LastName;
          patient.PatientCode = res.Results.PatientCode;
          patient.DateOfBirth = res.Results.DateOfBirth;
          patient.Gender = res.Results.Gender;
          patient.PatientId = res.Results.PatientId;
          patient.PhoneNumber = res.Results.PhoneNumber;
