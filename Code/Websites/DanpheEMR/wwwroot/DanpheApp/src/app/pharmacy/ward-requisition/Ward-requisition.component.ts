...
@Component({
    templateUrl: "../../view/pharmacy-view/WardRequisition/WardRequisition.html"  //"/PharmacyView/WardRequisition"
})
    ...
    public SetWardReqItemsData(selectedData) {
        if (selectedData) {
            this.wardSupplyBLService.GetWardReqItemList(selectedData.RequisitionId)
                .subscribe(res => {
                    if (res.Status == "OK") {
                        let temp =  new Array<WardDispatchItemsModel>();
                        temp = res.Results;
                        this.wardReqDispatch.RequisitionId = selectedData.RequisitionId;
                        for (let i = 0; i < temp.length; i++) {
                            temp[i].BatchNo = this.ItemListForSale.find(a => a.ItemId == temp[i].ItemId).BatchNo;
                            temp[i].ExpiryDate = this.ItemListForSale.find(a => a.ItemId == temp[i].ItemId).ExpiryDate;
                            temp[i].TotalQty = this.ItemListForSale.find(a => a.ItemId == temp[i].ItemId).AvailableQuantity;
                            temp[i].MRP = this.ItemListForSale.find(a => a.ItemId == temp[i].ItemId).MRP;

                        }

                        this.WardReqItemsList = temp;
