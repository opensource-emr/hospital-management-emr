...
export class NursingBLService {
  constructor(public nursingDLService: NursingDLService,
  ...
  public GetNursingOrderListByPatientId(patientId) {
    return this.nursingDLService.GetNursingOrderListByPatientId(patientId)
      .map((responseData) => {
        return responseData;
      })
  }
  //public Method: Post all (Lab, Imaging and all other department items) Nursing Order Items to Billing Requisition 
  public PostBillingRequisitionItems(billTranItems: Array<BillingTransactionItem>) {
    let billReqItems = this.GetMap_ReqItemsFromTxnItems(billTranItems);
    return this.nursingDLService.PostBillingReqItems(billReqItems)
      .map((responseData) => {
        return responseData;
      });
  }
  //public Method: Map all Nursing Items (originally we use billingTransactionItem model ) for Billing Transaction Requisition
  public GetMap_ReqItemsFromTxnItems(billItems: Array<BillingTransactionItem>): Array<BillItemRequisition> {
    let billReqItemsTemp: Array<BillItemRequisition> = billItems.map(function (item) {
  ...
  public GetOPDList() { //fromDate,toDate
    ...
    return this.visitDLService.GetPastVisits()
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public GetAdmittedList() {
    return this.admissionDLService.GetAdmittedList()
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public PostNursingOrderToDept(billingTransactionItems: Array<BillingTransactionItem>): Observable<any> {
    ...
    if (labItms && labItms.length > 0) {
      deptHttpRequests.push(this.nursingDLService.PostLabItemsRequisition(labItms).map(res => res));
      dptRequestIndexes.push({ dptName: "lab", index: currIndex });
      currIndex++;
    }
    if (imgItems && imgItems.length > 0) {
      deptHttpRequests.push(this.nursingDLService.PostImagingItemsRequest(imgItems).map(res => res));
  ...
  public PostDrugsRequisition(drugReq: DrugsRequisitonModel) {
    try {
      let newDeugReq: any = _.omit(drugReq, ['selectedPatient.PHRMPatientValidator']);
      ...
      return this.nursingDLService.PostDrugsRequisition(data)
        .map((res) => { return res });
    }
  ...
  public GetImagingItemsMapped(billItems: Array<BillingTransactionItem>): Array<ImagingItemRequisition> {
    let currentUser: number = this.securityService.GetLoggedInUser().EmployeeId;//logged in doctor
  ...
  public CancelRadRequest(item: BillingTransactionItem) {
    ...
    return this.nursingDLService.CancelRadRequest(data)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public CancelBillRequest(item: BillingTransactionItem) {
    var temp = _.omit(item, ['ItemList', 'BillingTransactionItemValidator', 'Patient']);
    let data = JSON.stringify(temp);
    return this.nursingDLService.CancelBillRequest(data)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public GetNephrologyPatients() {
    return this.nursingDLService.GetNephrologyPatients()
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public SubmitHemoReport(newHemoReport) {
    return this.nursingDLService.SubmitHemoReport(newHemoReport)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public CheckForLastReport(patientId) {
    return this.nursingDLService.CheckForLastReport(patientId)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public PreviousReportList(patientId) {
    return this.nursingDLService.PreviousReportList(patientId)
      .map((responseData) => {
        return responseData;
      });
  }
