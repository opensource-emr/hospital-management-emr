...
import { Input, Component, Output, ElementRef, ViewChild, ChangeDetectorRef, EventEmitter } from "@angular/core";
...
export class LabTestsViewReportFormat2Component {
  ...
  @Output("callbackBackToGrid") callbackAddUpdate: EventEmitter<object> = new EventEmitter<object>();
  ...
  public allColumnsCombined: LabReportColumnsModel = new LabReportColumnsModel(false, false, false, false, false, false, false);
  ...
  constructor(public labBLService: LabsBLService,
    ...
    public sanitizer: DomSanitizer, public coreService: CoreService, public router: Router,
  ...
  public set tempReport(_templateReport: LabReportVM) {
    ...
    if (_templateReport) {
      ...
      else if (_templateReport.Lookups.VisitType.toLowerCase() == 'inpatient') {
      ...
      if (_templateReport.Templates.length) {
        _templateReport.Templates.forEach(temp => {
          if (!_templateReport.FooterText || _templateReport.FooterText.trim() == '') {
      ...
      if (_templateReport.TemplateType == 'html') {
        ...
        if (_templateReport.Templates.length) {
          if (this.showHideHighLowNormalFlag && _templateReport.Templates[0].TemplateType == 'normal') {
          ...
          _templateReport.Templates.forEach(temp => {
            ...
            if (temp.TemplateType && temp.TemplateType.toLowerCase() == "culture") {
              if (temp.Tests != null && temp.Tests.length > 0) {
                temp.Tests.forEach(tst => {
                  tst.ResultFormattedForCulture = LabReportVM.FormatResultForCulture(tst);
                });
              }
            }
          });
          ...
          for (var pr in this.allColumnsCombined) {
            if (this.allColumnsCombined[pr]) { this.allColumnsCount = this.allColumnsCount + 1; }
          }
        }
  ...
  public GetDoctorsList() {
    this.labBLService.GetDoctorsList()
      .subscribe(res => {
        if (res.Status == 'OK') {
  ...
  PostLabReport(exportType: string = null) {
    if (this.CheckSignatoriesValidation()) {
      ...
      this.templateReport.Lookups.ReceivingDate = labReport.ReceivingDate = moment(new Date(Math.min.apply(null, rcvDates))).format('YYYY-MM-DD HH:mm');
      ...
      if (this.showLoggedInUserSignatory) {
        ...
        if (this.CreatedByUser.LabSignature && this.CreatedByUser.LabSignature.trim() != '') {
          var dupSign = signObj.find(itm => itm.EmployeeId == currUser.EmployeeId);
      ...
      this.labBLService.PostLabReport(labReport)
        .subscribe(res => {
          if (res.Status == "OK") {
  ...
  UpdateLabReport() {
    if (this.CheckSignatoriesValidation()) {
      ...
      this.labBLService.PutLabReport(labReport)
        .subscribe(res => {
          if (res.Status == "OK") {
            this.ParseSignatories(labReport.Signatories);
            this.loading = false;
  ...
  printLabReport() {
    ...
    if (this.templateReport.ReportId && this.loading && !this.printDirectlyFromGrid) {
      this.labBLService.UpdateIsPrintedFlag(this.templateReport.ReportId)
        .subscribe(res => {
          if (res.Status == "OK") {
            ...
            this.msgBoxServ.showMessage('failed', ["Error In Updating Print Informations in Report Table"]);
            this.loading = false;
            this.printDirectlyFromGrid = false;
          }
        });
  ...
  public print() {
    ...
    if (this.printDirectlyFromGrid) {
      this.printDirectlyFromGrid = false;
      this.callbackAddUpdate.emit({ printed: true });
    }
    this.loading = false;
    this.printDirectlyFromGrid = false;
  }
  ...
  public exportToWord() {
    ...
    var originalSource = document.getElementById("headerImg").getAttribute("src");
    ...
    //ashim: 01Sep2018 : We're now grouping by only sample code.
    a.download = this.templateReport.Lookups.PatientName + "-" + moment().format("YYMMDDHHmm");
  ...
  public exportToPdf() {
    ...
    //ashim: 01Sep2018 : We're now grouping by only sample code.
    var docName = this.templateReport.Lookups.PatientName + "-" + moment().format("YYMMDDHHmm") + '.pdf';
  ...
  public ShowHidePrintForTestsWithComp(template: LabResult_TemplatesVM, test: LabResult_TestVM) {
  ...
  public ShowHidePrintForTestsWithoutComp(template: LabResult_TemplatesVM, test: LabResult_TestVM, len: number) {
  ...
  public ShowHideComponentsToPrint(template: LabResult_TemplatesVM, test: LabResult_TestVM) {
    test["Print"] = false;
  ...
  UpdateSpecimen(test: LabResult_TestVM) {
    ...
    if (specimen && specimen.trim().length > 0) {
      this.labBLService.PutSpecimen(specimen, reqId)
        .subscribe(res => {
          if (res.Status == 'OK') {
            test.Specimen = res.Results;
            this.msgBoxServ.showMessage("success", ["Specimen updated successfully."]);
          }
          else {
            this.showChangeSample = false;
            this.showConfirmationBox = false;
      ...
      this.labBLService.GetSpecimen(reqId)
        .subscribe(res => {
          if (res.Status == 'OK') {
            test.Specimen = res.Results;
          }
  ...
  Verify() {
    ...
    if (this.CheckSignatoriesValidation()) {
      ...
      this.labBLService.VerifyAllLabTests(labReport)
        .subscribe(res => {
          if (res.Status == "OK") {
            this.callbackAddUpdate.emit({ verified: true });
            this.loading = false;
          } else {
            this.msgBoxServ.showMessage("failed", ['Unable to Verify the report.']);
            console.log(res.ErrorMessage);
            this.loading = false;
          }
        });
