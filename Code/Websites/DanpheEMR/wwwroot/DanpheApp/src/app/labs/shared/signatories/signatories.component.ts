...
export class SignatoriesComponent {
    ...
    LoadEmployees() {
        this.http.get<any>("/api/Master?type=signatories&departmentName=" + this.departmentName, this.options)
            .map(res => res)
            .subscribe((res: DanpheHTTPResponse) => {
                if (res.Results) {
                    this.allEmployees = res.Results;
                    this.allEmployees.sort(function (a, b) { return a.DisplaySequence - b.DisplaySequence });
                    this.allEmpsFormatted = this.allEmployees.map(emp => {
                        return { "id": emp.EmployeeId, "itemName": emp.FullName, "DisplaySequence": emp.DisplaySequence ? emp.DisplaySequence : 1000 };
                    });
                    ...
                    //ashim: 06Sep2018 displaying default signatoire in lab
                    else if (this.defaultEmployeeIdList && this.defaultEmployeeIdList.length) {
                        this.selEmployees = [];
    ...
    cancel(employeeId: number) {
        let index: number = this.selEmployees.findIndex(emp => emp.id == employeeId);
        if (index>=0) {
    ...
    GetItems(): Array<any> {
        ...
        this.selEmployees.sort(function (a, b) { return a.DisplaySequence - b.DisplaySequence });
        let retList = this.selEmployees.map(emp => {
            //changed
            let currEmpId = emp.id;
            if (this.departmentName == 'lab') {
                let currEmp = this.allEmployees.find(itm => itm.EmployeeId == currEmpId);
                if(currEmp){
                    return { EmployeeId: currEmp.EmployeeId, EmployeeFullName: currEmp.FullName, Signature: currEmp.LabSignature, DisplaySequence: currEmp.DisplaySequence ? currEmp.DisplaySequence : 1000  };
                }
            ...
            //changed
                ...
                let currEmp = this.allEmployees.find(itm => itm.EmployeeId == currEmpId);
                return { EmployeeId: currEmp.EmployeeId, EmployeeFullName: currEmp.FullName, Signature: currEmp.LongSignature, DisplaySequence: currEmp.DisplaySequence ? currEmp.DisplaySequence : 1000   };
            }
    ...
    SetItems(ipJson): Array<any> {
        ...
        ipList.forEach((item, ind) => {
            let currEmp = this.allEmployees.find(itm => itm.EmployeeId == item.EmployeeId);
            if(!currEmp){
                ipList.splice(ind,1);
            } else {
        ...
        let retList = ipList.map(emp => {
            ...
            index = index+1;
            if(currEmp){
                return { "id": currEmp.EmployeeId, "itemName": currEmp.FullName, "DisplaySequence": currEmp.DisplaySequence ? currEmp.DisplaySequence : 1000 };
            }             
        });
