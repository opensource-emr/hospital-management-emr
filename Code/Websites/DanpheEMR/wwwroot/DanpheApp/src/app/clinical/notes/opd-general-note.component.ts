...
import { IcdWithOrdersViewModel } from "../shared/all-icd-with-orders.viewmodel";
import { DiagnosisOrderVM } from "../shared/assessment-and-plan.model";
import { ICD10 } from "../shared/icd10.model";
import { PatientOrderListModel } from "../shared/order-list.model";
...
export class OPDGeneralNoteComponenet {
    ...
    MapOPDGeneralNote(_opdGeneral) {
        ...
        if (this.opdGeneralNote.SubjectiveNote) {
            this.opdGeneralNote.SubjectiveNote = new SubjectiveNote();
        ...
        if (this.opdGeneralNote.ObjectiveNote) {
            this.opdGeneralNote.ObjectiveNote = new ObjectiveNote();
    ...
    SaveOPDGeneralNote(): void {
        ...
        if (this.CheckValidations() && this.CheckIfAnyData()) {
            ...
            var temp = _.omit(this.opdGeneralNote, SubjectiveNote ? ['SubjectiveNote.SubjectiveNoteValidator'] : null);
    ...
    CheckIfAnyData(): boolean {
        ...
        if (!isValidObjective && !isValidSubjective && !isValidInstructions) {
            this.Initialize();
