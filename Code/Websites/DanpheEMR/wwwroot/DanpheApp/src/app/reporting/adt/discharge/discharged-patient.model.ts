...
export class RPT_ADT_DischargedPatientModel {
    ...
    public IsDirty(fieldName): boolean {
        if (fieldName == undefined)
            return this.DischargePatientValidator.dirty;
        else
            return this.DischargePatientValidator.controls[fieldName].dirty;
    }

    public IsValid():boolean{if(this.DischargePatientValidator.valid){return true;}else{return false;}} public IsValidCheck(fieldName, validator): boolean {
        if (fieldName == undefined)
            return this.DischargePatientValidator.valid;
        else
            return !(this.DischargePatientValidator.hasError(validator, fieldName));
    }
}
