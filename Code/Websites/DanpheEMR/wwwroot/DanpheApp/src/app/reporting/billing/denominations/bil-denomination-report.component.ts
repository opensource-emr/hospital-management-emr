...
export class RPT_BIL_BilDenominationReportComponent {
    ...
    Load() {
        ...
        if (this.currentDenominationReport.fromDate != null && this.currentDenominationReport.toDate != null) {
            this.dlService.Read("/Reporting/BilDenominationReport?FromDate=" + this.currentDenominationReport.fromDate +
    ...
    SelectedUser() {
        ...
        if (this.selUser) {
            if (typeof (this.selUser) == 'string' && this.userList) {
                user = this.userList.FullName.toLowerCase() == this.selUser.toLowerCase();
            }
