...
import { BillItemVM } from "../../billing/shared/discharge-bill.view.model";
import * as _ from 'lodash';
@Component({
    selector: 'autoBillingItem-list',
    templateUrl: './autoBillingItem-list.html',
})
export class AutoBillingItemListComponent {
    ...
    getAutoBillingItemList() {
        this.settingsBLService.GetAutoBillingItemList()
            .subscribe(res => {
                if (res.Status == "OK") {
    ...
    PostParameterValue() {
        this.settingsBLService.PostParameterValue()
            .subscribe(res => {
                if (res.Status == "OK") {
                    this.enable = false;
