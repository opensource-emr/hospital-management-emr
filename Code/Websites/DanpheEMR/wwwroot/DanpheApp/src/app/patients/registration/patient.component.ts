...
import { MembershipType } from "../shared/membership-type.model";
@Component({
  templateUrl: "../../view/PatientView/Patient.html" //"/PatientView/Patient"
})


export class PatientComponent implements IRouteGuard {
  // binding logic
  ...
  //public patientMembership: PatientMembership = new PatientMembership();
  ...
  GetMembershipType() {
    this.patientBLService.GetMembershipType()
      .subscribe(res => {
        if (res.Status == 'OK') {
          this.MembershipTypeList = res.Results;
          if (this.model.PatientId) {
            let type = this.MembershipTypeList.find(i => i.MembershipTypeId == this.model.MembershipTypeId);
            ...
            let index = this.MembershipTypeList.findIndex(i => i.MembershipType == "General");
            this.model.MembershipTypeId = this.MembershipTypeList[index].MembershipTypeId;
            this.membership = this.MembershipTypeList[index].MembershipTypeName;
            this.patientService.globalPatient.MembershipTypeName = this.MembershipTypeList[index].MembershipType;
            this.patientService.globalPatient.MembershipDiscountPercent = this.MembershipTypeList[index].DiscountPercent;
          }
        } else {
          this.msgBoxServ.showMessage("error", [res.ErrorMessage]);
  ...
  onMembershipTypeChange() {
    ...
    if (this.membership) {
      if (typeof (this.membership) == 'string') {
        type = this.MembershipTypeList.find(a => a.MembershipTypeName == this.membership);
      }
      else if (typeof (this.membership) == 'object') {
        type = this.membership;
      }
      if (type) {
        this.model.MembershipTypeId = this.membership.MembershipTypeId;
