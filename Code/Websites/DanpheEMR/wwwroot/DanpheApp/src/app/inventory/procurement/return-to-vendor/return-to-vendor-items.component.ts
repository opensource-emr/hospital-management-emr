...
import { ReturnToVendorModel } from '../../shared/return-to-vendor.model';
...
SetCurrentFiscalYear() {
  ...
  GetItemList() {
    if (this.vendorId > 0 && this.GoodsReceiptNo > 0 && this.selFiscYrId >0) {
      ...
      this.inventoryBLService.GetItemListbyVendorId(this.vendorId, this.GoodsReceiptNo, this.selFiscYrId)
        .subscribe(res => {
          if (res.Status == "OK") {
            ...
            if (this.itemList.length == 0) {
              this.messageBoxService.showMessage("notice", ['No items available']);
            }
          }
          else {
