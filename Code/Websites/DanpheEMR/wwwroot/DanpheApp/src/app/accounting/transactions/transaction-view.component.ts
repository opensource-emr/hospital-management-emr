...
export class TransactionViewComponent {
    ...
    constructor(public accBLService: AccountingBLService,
    ...
    public GetTxnbyVoucher(vouchernumber: string) {
        try {
            this.accBLService.GetTransactionbyVoucher(vouchernumber)
                .subscribe(res => {
                    if (res.Status == "OK") {
                        let data = res.Results;
                        if (vouchernumber.includes("SV") && data.SectionId == 2) {
