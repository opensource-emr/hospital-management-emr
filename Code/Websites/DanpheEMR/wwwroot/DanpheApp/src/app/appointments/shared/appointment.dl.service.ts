...
export class AppointmentDLService {
    ...
    public GetAppointmentList(status: string) {
        return this.http.get<any>("/api/Appointment?&reqType=getAppointments"
            + "&status=" + status);
    }
    ...
    public PutAppointmentStatus(appointmentId: number, status: string) {
