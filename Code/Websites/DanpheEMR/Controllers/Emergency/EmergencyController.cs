            ...
            using (var emergencyDbContextTransaction = _emergencyDbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if ((EnableProvRegistration != null) && (EnableProvRegistration == "1" || EnableProvRegistration.ToLower() == "true"))
                    {
                        ...
                        var BillItemPrice = (from blitm in _emergencyDbContext.BillItemPrice
                                                       ...
                                                       && blitm.ServiceDepartmentId == ServiceDepartmentId && priceCatServItem.PriceCategoryId == 1 //Krishna, 13thMarch'23, 1 is for Normal and Hardcoded for Now
                                                       select new { 
                                                           blitm, 
                                                           Price = priceCatServItem.Price }).FirstOrDefault();
                        billItem.ItemId = BillItemPrice.blitm.IntegrationItemId;
